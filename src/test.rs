...
fn test_list() {
    ...
    let write_result = list.write(Path::new("test.json"), false);
    assert!(
        write_result.is_ok(),
        "Failed to write list to file: {:?}",
        write_result.err()
    );
    let load_result = list.load(Path::new("test.json"));
    fs::remove_file("test.json").unwrap_or_else(|e| {
    ...
    // create new file
    let write_result = list.write(Path::new("test.json"), true);
    assert!(
        write_result.is_ok(),
        "Failed to write list to file: {:?}",
        write_result.err()
